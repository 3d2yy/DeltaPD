...
from deltapd.campaign.aggregate import summarize_metrics
from deltapd.campaign.config import load_config
from deltapd.campaign.detection_curves import compute_detection_curves, summarize_detection_curves
...
def run_thesis_campaign(config_path: str | Path) -> dict[str, pd.DataFrame]:
    ...
    repo_root = resolved.parents[1] if len(resolved.parents) >= 2 else resolved.parent
