...
def plot_classification_timeline(df_delta: pd.DataFrame, out_png: str):
    """
    Grafica la categoría predicha y la confianza a lo largo del tiempo.
    """
    if "pred_category" not in df_delta.columns:
        return
        
    fig, (ax1, ax2) = plt.subplots(2, 1, figsize=(10, 7), sharex=True)
    t_vals = df_delta["toa_s"]
    
    # Mapeo de categorías a valores numéricos para plotting
    cat_map = {'Corona': 3, 'Superficial': 2, 'Interna': 1, 'Ruido': 0, 'Unknown': -1}
    y_cats = [cat_map.get(c, -1) for c in df_delta["pred_category"]]
    
    ax1.step(t_vals, y_cats, where='post', color='darkgreen', linewidth=1.5)
    ax1.set_yticks(list(cat_map.values()))
    ax1.set_yticklabels(list(cat_map.keys()))
    ax1.set_ylabel("Categoría Predicha")
    ax1.set_title("Evolución de la Clasificación de DP")
    ax1.grid(True, linestyle="--", alpha=0.3)
    
    ax2.plot(t_vals, df_delta["pred_confidence"], color='blue', alpha=0.7)
    ax2.set_ylabel("Confianza (Score)")
    ax2.set_xlabel("Tiempo real del ensayo (s)")
    ax2.set_ylim(0, 1.1)
    ax2.grid(True, linestyle="--", alpha=0.3)
    
    _add_stage_shading(ax1, df_delta, is_time=True)
    _add_stage_shading(ax2, df_delta, is_time=True)
